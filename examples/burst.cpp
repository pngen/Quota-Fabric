...
int main() {
  ...
  ef.set_tenant_quota(t, QV(t, 0, 2048, 4096, 1024)); auto a = AUTH(ef);
  auto r = ef.reserve(t, V(3072), a); std::printf("burst reserve 3072: %s\n", r.ok?"ALLOW_BURST":"DENY");
  auto env = ef.envelope(t); std::printf("burst_use=%lld active=%d\n", (long long)env.burst_usage.get(ResourceDimension::AcceleratorVRAM), env.burst_active?1:0);
  auto r2 = ef.reserve(t, V(1024), a); std::printf("second burst 1024 (exhausted): %s (%s)\n", r2.ok?"ALLOW":"DENY", to_string(r2.status.code).data());
  return 0; }