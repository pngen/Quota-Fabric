...
int main() {
  ...
  ef.set_tenant_quota(t, QV(t, 4096, 8192, 8192)); auto a = AUTH(ef);
  auto r = ef.reserve(t, V(4096), a); auto env = ef.envelope(t);
  std::printf("guaranteed reserve allowed=%d available_guaranteed=%lld\n", r.ok?1:0, (long long)env.available_guaranteed.get(ResourceDimension::AcceleratorVRAM));
  auto r2 = ef.reserve(t, V(4096), a); std::printf("above guarantee within soft: %s\n", r2.ok?"ALLOW":"DENY");
  return 0; }