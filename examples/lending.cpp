...
int main() {
  ...
  auto a = AUTH(ef); auto ld = ef.lend(l, b, V(1024), a);
  std::printf("lend 1024: %s\n", ld.ok?"OK":"DENY");
  auto lv = ef.envelope(l); std::printf("lender lent=%lld\n", (long long)lv.lent.get(ResourceDimension::AcceleratorVRAM));
  return 0; }