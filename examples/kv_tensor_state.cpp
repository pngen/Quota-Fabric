...
int main() {
  ...
  ResourceQuota q = Q(t); q.guaranteed = KV(1<<30); q.soft_limit = q.guaranteed; q.hard_limit = q.guaranteed; ef.set_tenant_quota(t, q);
  ef.apply_observation(OBS(t, ResourceDimension::KVCacheBytes, 512<<20, ObservationKind::KV_USAGE_CHANGE));
  auto env = ef.envelope(t); std::printf("KV state consumed=%lld / quota=%lld\n", (long long)env.current_consumption.get(ResourceDimension::KVCacheBytes), (long long)env.guaranteed.get(ResourceDimension::KVCacheBytes));
  return 0; }