...
int main() {
  ...
  ResourceQuota q = Q(t); q.guaranteed = TB(1<<30); q.soft_limit = q.guaranteed; q.hard_limit = q.guaranteed; ef.set_tenant_quota(t, q);
  auto before = ef.evaluate_reservation(t, TB(256<<20)).code;
  ef.apply_observation(OBS(t, ResourceDimension::TransferBytes, 900<<20, ObservationKind::TRANSFER_CONSUMED));
  auto after = ef.evaluate_reservation(t, TB(256<<20)).code;
  std::printf("transfer before=%s after=%s\n", to_string(before).data(), to_string(after).data());
  return 0; }