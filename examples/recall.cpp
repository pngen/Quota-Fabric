...
int main() {
  ...
  auto a = AUTH(ef); auto br = ef.borrow(b, V(2048), a); if (!br.ok) return 1;
  auto rec = ef.recall_decision(l); std::printf("recall decisions:");
  for (auto r : rec) std::printf(" %s", to_string(r).data()); std::printf("\n");
  ef.recall_borrow(br.value.id, a);
  auto env = ef.envelope(l); std::printf("after recall lender lent=%lld\n", (long long)env.lent.get(ResourceDimension::AcceleratorVRAM));
  return 0; }