...
int main() {
  ...
  ResourceQuota q = Q(t); q.guaranteed = CT(5000); q.soft_limit = CT(5000); q.hard_limit = CT(5000); ef.set_tenant_quota(t, q);
  auto before = ef.evaluate_reservation(t, CT(1000)).code;
  ef.apply_observation(OBS(t, ResourceDimension::AcceleratorComputeTime, 4800, ObservationKind::COMPUTE_INTERVAL));
  auto after = ef.evaluate_reservation(t, CT(1000)).code;
  std::printf("compute budget before=%s after=%s\n", to_string(before).data(), to_string(after).data());
  return 0; }