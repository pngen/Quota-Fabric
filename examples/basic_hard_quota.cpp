...
int main() {
  ...
  ef.set_tenant_quota(t, QV(t, 4096, 4096, 4096)); auto a = AUTH(ef);
  auto r = ef.reserve(t, V(1024), a); std::printf("reserve 1024MB: %s (%s)\n", r.ok?"ALLOW":"DENY", to_string(r.status.code).data());
  auto r2 = ef.reserve(t, V(4096), a); std::printf("reserve 4096MB (exact hard): %s\n", r2.ok?"ALLOW":"DENY");
  auto r3 = ef.reserve(t, V(1), a); std::printf("reserve 1MB over hard: %s (%s)\n", r3.ok?"ALLOW":"DENY", to_string(r3.status.code).data());
  return 0; }